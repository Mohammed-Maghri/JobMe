...
The Prisma client is generated into `lib/generated/prisma`, which is
git-ignored — a fresh clone does not contain it. It is generated by the
`postinstall` script, and `npm run build` runs `prisma generate` again so the
build does not depend on install hooks having run (some CI setups pass
`--ignore-scripts`).

Set `DATABASE_URL`, `BETTER_AUTH_SECRET` and `BETTER_AUTH_URL` in the host's
environment before building. `lib/env.ts` reads them at import time, so a
missing one fails the build with an explicit message rather than at runtime.
`BETTER_AUTH_URL` must be the deployed origin, and the Google redirect URI in
Cloud Console has to match it.

Run `npx prisma migrate deploy` against the production database; the build does
